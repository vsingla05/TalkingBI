...
const COLORS = ["#a78bfa", "#00bcd4", "#4caf8a", "#ff9800", "#e91e63", "#6c63ff", "#34d399"];

export default function InsightsDashboard({ charts, dashboardData = null, onVoiceSummary, isPlayingAudio = false, isPausedAudio = false, onStopAudio, onPauseAudio, onResumeAudio }) {
  ...
  const handleVoiceSummary = async () => {
    ...
    try {
      const summary = {
        ...
        insight: dashboardData?.insight || "Key discoveries and patterns",
      };
      await onVoiceSummary("insights", summary);
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12 px-6 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-white">{dashboardData?.title || "Insights Dashboard"}</h1>
          <p className="text-[#8a8fa8] mt-2">{dashboardData?.insight || "Key discoveries and patterns"}</p>
        </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all ${
              isPlaying
                ...
                : "bg-gradient-to-r from-[#a78bfa] to-[#00bcd4] hover:shadow-lg hover:shadow-[#a78bfa]/30"
      ...
      {displayKPIs.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {displayKPIs.map((kpi, idx) => (
            <div key={idx} className="bg-gradient-to-br from-[#1e2029] to-[#16181f] border border-[#2a2d3a] rounded-2xl p-6 hover:border-[#a78bfa]/30 transition-all">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    {chart.type === "area" && (
                      <AreaChart data={chart.data}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: "#16181f", border: "1px solid #2a2d3a", borderRadius: "8px", color: "#f0f0f5" }} />
                        <Legend />
                        <Area type="monotone" dataKey={chart.y_axis} stroke="#a78bfa" fill="#a78bfa" fillOpacity={0.3} />
                      </AreaChart>
                    )}
                    {chart.type === "pie" && (
                      <PieChart>
                    ...
                    {chart.type === "scatter" && (
                      <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 10 }}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: "#16181f", border: "1px solid #2a2d3a", borderRadius: "8px", color: "#f0f0f5" }} />
                        <Scatter name="Data" data={chart.data} fill="#e91e63" />
                      </ScatterChart>
                    )}
                  </ResponsiveContainer>
                ) : (
